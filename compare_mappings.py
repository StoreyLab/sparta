...
def compare_mappings(samfile1, samfile2, genome1_name, genome2_name, genome1_prior=0.5, genome2_prior=0.5, posterior_cutoff = 0.9):
    ...
    def log10_matched_base_prob(phred_char):
        if phred_char not in log10_matched_base_probs:
            log10_matched_base_probs[phred_char] = math.log10(1.0 - math.pow(10, (ord(phred_char) - 33) * -0.1))
        return log10_matched_base_probs[phred_char]
    ...
    def log10_mismatched_base_prob(phred_char):
        if phred_char not in log10_mismatched_base_probs:
            log10_mismatched_base_probs[phred_char] = ((ord(phred_char) - 33) * -0.1) - math.log10(3.0)
        return log10_mismatched_base_probs[phred_char]

    
    # compute probability of a whole RNAseq read being generated by a genome, 
    # given the genome in question and the quality scores
    def compute_sam_prob(sam):
        ...
        sam_err = re.findall(MD_REGEX, sam.opt("MD"))        
        total = 0
        seq_ix = 0
        # step through sequence
        while sam_err:
            
            matched_bases, err = sam_err.pop()
            ...
            for match in range(0, int(matched_bases)):
                total += log10_matched_base_prob(sam.qual[seq_ix])
                seq_ix += 1
            ...
            # step through mismatched bases and sum log10 probabilities of that mismatch occuring
                ...
                for mismatched_base in err:
                    total += log10_mismatched_base_prob(sam.qual[seq_ix])                
                    seq_ix += 1
        
        # step through the remaining matched bases
        while seq_ix < len(sam.qual):
            total += log10_matched_base_prob(sam.qual[seq_ix])
            seq_ix += 1
    ...
    sam_obj1 = pysam.Samfile(samfile1)
    sam_obj2 = pysam.Samfile(samfile2)
    ...
    for sam1, sam2 in zip(sam_obj1, sam_obj2):
        
        assert sam1.qname == sam2.qname     
        
        if sam1.is_unmapped and sam2.is_unmapped:
            #this read is probably junk
            no_match += 1
            continue
        elif sam1.is_unmapped != sam2.is_unmapped:
            #maps to one but not the other; either junk or
            #an unshared gene bw BY and RM
            match1 += 1
            continue
        elif sam1.opt("MD") == sam2.opt("MD"):
            # same errors
            same_errors += 1
        else:
            # bowtie matched the read to both genomes
            prob_read_genome1 = compute_sam_prob(sam1)
            prob_read_genome2 = compute_sam_prob(sam2)
            ...
            if (prob_genome1 >= posterior_cutoff):
                print '{}\t{}'.format(sam1.qname, genome1_name)
                match_genome1_after_analysis += 1
            elif (prob_genome2 >= posterior_cutoff):
                print '{}\t{}'.format(sam1.qname, genome2_name)
                match_genome2_after_analysis += 1
            else:
                print '{}\tunmapped'.format(sam1.qname)
